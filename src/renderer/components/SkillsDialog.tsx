    ...
    <div className="settings-overlay" onClick={onClose}>
      <div className="md-dialog" onClick={e => e.stopPropagation()} style={{ left: pos.x, top: pos.y, width: size.w, height: size.h }}>
        <div className="md-header" onMouseDown={onDragStart}>
          <h2>技能库</h2>
          <button className="settings-btn-sm" onClick={async () => { setRefreshing(true); try { await window.electronAPI?.reloadSkills?.(); loadSkills(); } catch {} finally { setRefreshing(false); } }} style={{ marginRight: 8, fontSize: 11 }} disabled={refreshing}>{refreshing ? <span className="spinning" style={{display: "inline-block"}}>&#x27F3;</span> : "⟳"} {refreshing ? "刷新中..." : "刷新"}</button>
