...
export function useSpeechInput({ onResult, enabled = true }: UseSpeechInputOptions): UseSpeechInputReturn {
    ...
    if (!recognitionRef.current && SpeechRecognitionAPI) {
      ...
      rec.onresult = (event: any) => {
        ...
        for (let j = event.resultIndex; j < event.results.length; j++) {
          ...
          if (event.results[j].isFinal) { f += t; } else { i += t; }
        }
        if (f) {
  ...
  const start = useCallback(() => {
    ...
    if (!rec) return;
    try { rec.start(); listeningRef.current = true; setListening(true); setFinalText(''); } catch {}
  ...
  useEffect(() => {
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      holdTimerRef.current = window.setTimeout(() => { holdTimerRef.current = null; start(); }, HOLD_THRESHOLD);
    };
